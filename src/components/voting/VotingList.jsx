...
const VotingList = ({ address, name, activated, over, sign, create }) => {
  ...
  const getVotingList = () => {
    if (!(ethers.constants.AddressZero === address)) {
      const q = query(
        ...
        where("daoAddress", "==", address.toLowerCase())
      );
      onSnapshot(q, async (querySnapshot) => {
        try {
          let docs = querySnapshot.docs.map((doc) => ({ ...doc.data() }));
          const daoViewerContract = await getDaoViewerContract();
          ...
          if (docs.length > 0) {
            ...
            for (let doc of docs) {
              ...
              const shared = await daoViewerContract.getShare(address, users);
              const voting_duration = await daoContract.VOTING_DURATION();
            ...
            items = loadash.orderBy(
              items,
              ["active", "sign", "activated", "createdAt"],
              ["desc", "desc", "desc", "desc"]
            );
            setVotingList(items);
            if (items.length > 0) {
  ...
  return (
    <>
      {show && (
        <div className="myvoting text-white p-3 tabborder mb-3">
          {create && (
            <Row>
              <Col>
                <h4>My voting</h4>
          ...
          {votingList.map((voting, index) => (
            <div
              className="voting-hover px-3 py-3 my-1"
              onClick={() => moveToSignature(voting.txHash)}
              key={index}
            >
              <Row>
                <Col style={{ overflowWrap: "break-word" }}>
                  ...
                  {voting.sign && (
                    <button className="dao-warning-btn">IN PROGRESS</button>
                  )}
                  {voting.active && (
                  ...
                  {voting.over && (
                    <button className="outline-danger">VOTING IS OVER</button>
                  )}
                  {voting.activated && (
                    <button
                ...
                {!voting.over && (
                  <Col className="text-right">
                    <p className="p-0 m-0">
                      <span>QUORUM: </span>
                      <span>
                        {Math.round((voting.share / voting.totalSupply) * 100)}%
                      </span>
                    </p>
                    {!voting.activated && (
                      <p>{calculateDate(voting.createdAt)}</p>
                    )}
                  </Col>
                )}
              </Row>
