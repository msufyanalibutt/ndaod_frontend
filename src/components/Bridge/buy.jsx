...
const BuyBridge = ({
    ...
    const getEthsTokens = async () => {
        try {
            const result = await axios.get(`https://api.1inch.io/v4.0/${1}/tokens`);
    ...
    const getBnbTokens = async () => {
        try {
            const result = await axios.get(`https://api.1inch.io/v4.0/${56}/tokens`);
            let tokens = result.data.tokens;
            tokens = Object.entries(tokens).map(item => item[1]);
            setTokens(tokens);
            setBnbs(tokens);
    ...
    const getAaveTokens = async () => {
        try {
            const result = await axios.get(`https://api.1inch.io/v4.0/${43114}/tokens`);
            let tokens = result.data.tokens;
            tokens = Object.entries(tokens).map(item => item[1]);
            setAvves(tokens);
        } catch (error) {
        ...
        <Modal show={show} onHide={() => {
            ...
            <ModalBody >
                ...
                <Row>
                    <Col className='my-3'>
                        <div
                            className={`dao-btn ecosystem ${chain === 1 && 'selectedecosystem'}`}
                            onClick={() => getSelectedChainTokens(1)}
                        >Ethereum</div>
                    </Col>
                    <Col className='my-3'>
                        <div
                            className={`dao-btn ecosystem ${chain === 56 && 'selectedecosystem'}`}
                    ...
                    <Col className='my-3'>
                        <div
                            className={`dao-btn ecosystem ${chain === 43114 && 'selectedecosystem'}`}
                                    ...
                                    <Row>
                                        <Col>
                                            <div className='d-inline-flex align-items-center'>
                                                <div className='me-3'>
                                                    <GetImage url={item.logoURI} alttext={item.symbol} newStyle={{ maxWidth: '40px', width: '100%' }} />
                                                </div>
                                                <div>
                                                    <h6>{item.symbol}</h6>
