...
import { Row, Col, Container, Form, FormGroup, Spinner, Placeholder, FormLabel, FormControl } from 'react-bootstrap';
...
const Index = ({ senderAddress, searchParams }) => {
    ...
    const getInfo = async () => {
        try {
            ...
            let allowance = await contract.allowance(senderAddress, networks[chainId].symbosis);
    ...
    const getConversionRate = async (value, item) => {
        ...
        if (!value || value < 0) {
            ...
            try {
                ...
                let obj = {
                    deadline: Number(moment.duration(Date.now()).asSeconds().toFixed(0)) + (20 * 60),
                    from: senderAddress || '0x333330cd9c430fae7536ed9684f511f229527e09',
                    revertableAddress: senderAddress || '0x333330cd9c430fae7536ed9684f511f229527e09',
                    slippage: 300,
                    to: values.recipientAddress ? values.recipientAddress : senderAddress || '0x333330cd9c430fae7536ed9684f511f229527e09',
                    tokenAmountIn: {
                        decimals: values.fromdecimals,
                        chainId: chainId,
                        address: fromAddress === networks[chainId].inchCoin ? "" : fromAddress,
                        amount: amount
                    },
                    tokenOut: {
                ...
                let url = `https://api.symbiosis.finance/crosschain/v1/swapping/exact_in?partnerId=ndaod`;
    ...
    const handleFormSubmit = async (body) => {
        ...
        if (senderAddress !== account) {
            if (networks[chainId].inchCoin === fromAddress) {
                ...
                moveToSwapToken(iface, txn.value ? ethers.utils.parseEther(values.fromBalance)._hex : 0);
            } else {
                const iface = createTokenForInterface(fromAddress, ethers.utils.parseEther(String(values.fromBalance)), txn.data);
        ...
        try {
            ...
            const result = await contract.swapEth(txn.data, { value: ethers.utils.parseEther(values.fromBalance) });
            await result.wait();
            resetFields();
        } catch (error) {
    ...
    const formSchema = yup.object().shape({
        ...
        networkFee: yup.number().moreThan(-1, 'Must be more then 0').required('This is required'),
    ...
    const moveToApprovedToken = () => {
        ...
        navigate(`/approvedToken/${senderAddress}?targetAddress=${networks[chainId].symbosis}&tokenAddress=${fromAddress}&tokenAmount=${value}`, { replace: true })
    }
    const resetFields = () => {
        setErrMsg('');
        setTxn(null);
    ...
    const moveToSwapToken = async (data, value) => {
        ...
        navigate(`/customTransaction/${senderAddress}?targetAddress=${networks[chainId].symbosis}&data=${data}&value=${value}&title=Symbiosis: Cross-chain Swap ${fromName} to ${toName}&desc=Cross-chain Swap ${fromBalance} ${fromName} to ${toBalance} ${toName}`);
    }
                            ...
                            <div className='tabborder p-3'>
                                ...
                                <Row>
                                    ...
                                    <Col xs={12} className="text-right text-muted">
                                        {
                                            senderAddress !== account && <p>
                                                DAO has a different address on a different blockchain <br />
                                                Recipient Address required when swapping on behalf of a DAO
                                            </p>
                                        }
                                        {
                                            senderAddress === account && <p>
                                                Optional. If not filled, will be sent to the sender's address
                                            </p>
                                        }
                                    </Col>
                                </Row>
                                ...
                                <Col className="text-right">
                                    {
                                        loading ? <Placeholder as="p" animation="glow">
                                            <Placeholder xs={12} />
                                        </Placeholder> : <p className='p-0 m-0'>
                                            {
                                ...
                                <Col className="text-right">
                                    {
                                        loading ? <Placeholder as="p" animation="glow">
                                            <Placeholder xs={4} />
                                        </Placeholder> : <p className='p-0 m-0 text-danger'>{values.networkFee > -1 ? values.networkFee : 0}$</p>
                                    }
                                </Col>
                            </Row>
                        </FormGroup>
                        ...
                        </FormGroup>
                        {
                            approved ? <FormGroup className="mb-5">
                                <button
                                    ...
                                    style={{ backgroundColor: 'palegreen', color: '#0D0D15', fontSize: '20px', fontWeight: 'bold' }}
                                    disabled={!(isValid && dirty && senderAddress) || loading}
                                >
                                    {
                                        loading ? <Spinner animation="border" variant="success" /> : 'Cross-chain Swap'
                                    }
                                </button>
