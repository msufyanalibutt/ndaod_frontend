...
import depositeCoinList from './depositCoin.json';
...
const SellShift = ({setFieldValue}) => {
    ...
    const updateFormValues = ()=>{
        setFieldValue('depositeCoin',sellCoin.coin);
        setFieldValue('depositeNetwork',sellCoin.network)
    }
    const getListofCoins = async () => {
        try {
            ...
            setCoins(depositeCoinList);
            setTempCoins(depositeCoinList);
        } catch (error) {
        }
    }
    ...
    return <>
        <div className='tabborder w-100 text-center p-3 pointer d-flex flex-column' onClick={handleShow}>
            <h6>Deposit Network</h6>
            <GetImage
                url={`https://sideshift.ai/api/v2/coins/icon/${sellCoin.name}-${sellCoin.network}`}
                newStyle={{ maxWidth: '50%', width: '100%', margin: "0px auto 10px auto" }}
            />
            <div>{sellCoin.name}</div>
            <div className="text-uppercase text-muted mb-2">
                <small>{sellCoin.coin}</small>
            </div>
            <div
                className="text-uppercase d-inline-block py-1 px-2"
                style={{ border: '2px solid white', minWidth: '100px' }}>{sellCoin.network}
            </div>
        </div>
        ...
        <Modal show={show} fullscreen={true} onHide={handleClose}>
            <Modal.Body>
                ...
                <h3 className='text-center'>You Send</h3>
                <Row className='mb-3'>
                            ...
                            (coins && coins.length > 0) && coins.map((item) => (
                                item.networks.map((coin, index) => (
                                    <Col key={index} className="my-3 ">
                                        <div
                                            className={`text-center py-3 ecosystem ${`${item.name}-${coin}` === `${sellCoin.name}-${sellCoin.network}` && 'selectedecosystem'}`}
                                            style={{ outline: 'none' }}
                                            onClick={
                                                () => HandleSelectedCoin({
                                                    coin: item.coin,
