                        ...
                        <Container>
                            <Row>
                                <Col xs={12} xl={7} className="mx-auto py-3">
                                    <NavLink className="dao-btn btn d-block" to={`/createTa/${address}`}>Create Trading Account</NavLink>
                                </Col>
                            </Row>
                        </Container>
                        <Container>
                                    ...
                                    <Row>
                                        <Col>
                                            <span className='text-white'>
                                                {name}: {truncateAddress(tAddress)} <ClipBoard address={tAddress} />
                                            </span>
                                        </Col>
                                        <Col className="text-right text-white">
                                            <h6>AUM :  ${new Intl.NumberFormat('en-US', { maximumFractionDigits: 2, notation: "compact", compactDisplay: "short" }).format(taBalance)}</h6>
                                        </Col>
                                    </Row>
                                </Col>
                            </Row>
                            ...
                            <Row>
                                <Col xs={12} xl={7} className="mx-auto py-3">
                                    <div className='myassets text-white tabborder mb-3 p-3'>
                                        {
                                            account && <TradingAccountAsset owner={owner} assets={assets} chainId={chainId} address={address} tAddress={tAddress} />
                                        }
                                    </div>
                                </Col>
                            </Row>
                        ...
                        <Container>
                            <Row>
                                <Col xs={12} xl={7} className="mx-auto mb-3">
                                    <NavLink className="dao-btn d-block text-center" to={`/connectTA/${address}/${name}`}>
                                        <GiJoint /> Connect
                                    </NavLink>
                                </Col>
                            </Row>
                                ...
                                <Col xs={12} xl={7} className="mx-auto">
                                    {
                                        (members && members.length > 0) && members.map(
                                            (item, index) => (
                                                <Row xs={2} key={index} className="d-flex align-items-center">
                                                    <Col className="mb-3 text-white">
                                                        <NavLink className={'text-white'} to={`/profile/${item.address}`}>{truncateAddress(item.address)}{' '}<ClipBoard address={item.address} /></NavLink>
                                                    </Col>
                                                    <Col className="mb-3 text-white  text-right">
                                                        {owner && <DeleteTradingAccount address={address} tAddress={tAddress} owner={owner} member={item.address} />}
                                                    </Col>
                                                </Row>
                                            )
                                        )
