...
const ChangeLPBurning = () => {
    ...
    const instanceHandleSubmit = async () => {
        const { title, description, active } = values;
        if (!library) return;
        try {
            setiLoading(true)
            const contract = await dao(address);
    ...
    const createForIface = (_burnable) => {
        let ABI = ["function changeBurnable(bool _burnable)"];
        let iface = new ethers.utils.Interface(ABI);
        iface = iface.encodeFunctionData("changeBurnable", [_burnable]);
        return iface
    }
    const getLPBurningStatus = async () => {
        try {
            const contract = await dao(address);
            const lpAddress = await contract.lp();
            const lpContract = await getLpContract(lpAddress);
            const burnable = await lpContract.burnable();
            setFieldValue('active', burnable);
            setFieldValue('title', `Change LP Burnning Status: ${burnable ? 'Enabled' : 'Disabled'}`)
        } catch (error) {
        }
    }
                    ...
                    <Container className='my-3'>
                        <Row>
                            <Col xs={12} md={8} className="mx-auto text-white">
                                <h2>Change LP Burning Status</h2>
                                    ...
                                    <FormGroup className="mb-3" controlId="formBasicCheckbox">
                                        <FormCheck
                                            ...
                                            onChange={(e) => {
                                                ...
                                                setFieldValue('title', `Change LP Burning Status: ${checked ? 'Enabled' : 'Disabled'}`)
                                            }}
                                            onBlur={handleBlur}
                                        />
                                        <span className='text-muted'>Burning Enabled</span>
                                    </FormGroup>
...
export default ChangeLPBurning;
