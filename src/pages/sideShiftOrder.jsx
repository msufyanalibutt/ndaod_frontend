...
import { Container, Row, Col, Modal, Form, FormGroup, FormLabel, FormControl ,Spinner} from 'react-bootstrap';
...
const SideShiftOrder = () => {
    ...
    const getOrderDetail = async () => {
        try {
            setLoading(true);
            const result = await axios.get(`https://sideshift.ai/api/v2/shifts/${id}`);
            let order = result.data;
            if (order) {
                let i = depositCoins.filter(item => {
                    if (item['Network'] === String(order.depositNetwork).toUpperCase() && item['Coin'] === String(order.depositCoin).toUpperCase()) {
                        return true;
                    } else {
                        return false;
                    }
    ...
    const formSchema = yup.object().shape({
        amount: yup.number().test('minLength', `Minimum ${order && order.depositMin} ${order && order.depositCoin}`, function (value) {
            return value >= order.depositMin
        }).test('maxLength', `Maximum ${order && order.depositMax} ${order && order.depositCoin}`, function (value) {
        ...
        <Container className='py-3 text-white'>
            <Row>
                <Col xs={12} lg={6} className='mx-auto'>
                    {
                        order && <div className='tabborder p-3'>
                            <Row>
                                <Col>
                            ...
                            <h4 className='text-center'>Waiting for you to send {order.depositCoin}</h4>
                            <h5 className='text-primary'>To Address</h5>
                            <p>{truncateAddress(order.depositAddress)} <ClipBoard address={order.depositAddress} /></p>
                            <div>
                            ...
                            <div className='text-center my-3'>
                                <button 
                                type='button' className='dao-btn' onClick={handleShow}
                                disabled={depositCoin && (chainId !== Number(depositCoin.ChainId))}
                                >Send</button>
                            </div>
                            {
                            ...
                            <Modal centered show={show} onHide={handleClose}>
                                <Modal.Body>
                                    ...
                                    <div className='text-center'>
                                        <h5>Send {order.depositCoin} ({String(order.depositNetwork).toUpperCase()})</h5>
                                        <Form onSubmit={handleSubmit}>
                                            <FormGroup className='mb-3'>
                                                <FormLabel htmlFor='amount' as="h4">Amount</FormLabel>
                                                <FormControl
                                                    id="amount"
                                                    type="number"
                                                    name="amount"
                                            ...
                                            <FormGroup>
                                                ...
                                                >
                                                    {
                                                        loading?<Spinner animation="border" variant="info" /> :'Confirm'
                                                    }
                                                </button>
                                            </FormGroup>
                                        </Form>
