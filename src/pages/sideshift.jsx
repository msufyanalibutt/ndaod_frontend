...
import { Container, Row, Col, FormControl, Form, Spinner, Modal } from 'react-bootstrap';
...
import { ssAffiliateId, ssAffiliateSecret, truncateAddress, chainIds } from '../utils';
...
const SideShift = () => {
    ...
    const handleFormSubmit = async ({ senderAddress, recipientAddress, depositeCoin, settleCoin, depositeNetwork, settleNetwork }) => {
        try {
            setLoading(true);
                                ...
                                <Col className="mb-3">
                                    <div className={`d-flex align-items-center py-2 px-3 ecosystem ${selected === account && 'selectedecosystem'}`} onClick={() => selectedDao(account)}>
                                        ...
                                        <div>
                                            ...
                                            {active && <p className='p-0 m-0 text-white'>{truncateAddress(account)}</p>}
                                        </div>
                                    </div>
                                </Col>
                                {
                                    (daos && daos.length > 0) && daos.map((dao, index) => (
                            ...
                            <Form onSubmit={handleSubmit}>
                                <div className='tabborder p-5'>
                                    ...
                                    <FormControl
                                        ...
                                        placeholder={`Your ${buyCoin.coin} (${String(buyCoin.network).toUpperCase()}) Address`}
                                        onChange={handleChange}
                                        onBlur={handleBlur}
                                        value={values.recipientAddress}
                                    />
                                    <div className='text-center mb-3'>
                                        <button type='button' className='dao-btn' onClick={getAddress}>Get Address From Metamask</button>
                                    </div>
                                    <div className='text-center'>
                                        ...
                                        >
                                            {
                                                loading ? <Spinner animation="border" variant="primary" /> : 'Shift'
                                            }
                                    ...
                                    {
                                        (values.recipientAddress && (mselected && String(networks[mselected.chain].chainName).toUpperCase() !== String(buyCoin.network).toUpperCase())) && <div className='text-center mt-3'>
                                            <div className='dao-warning-btn text-white d-inline-block mx-auto px-3 py-2' style={{ fontSize: '16px' }}>
                                                Wrong Network. Select Dao On {String(buyCoin.network).toUpperCase()}! Chain.
                                            </div>
                                        </div>
                                    }
                                </div>
                    ...
                    <Modal size="lg" fullscreen={true} show={show} onHide={handleClose} >
                        <Modal.Body>
                            ...
                            <Container>
                                <Row>
                                    {
                                        (mdaos && mdaos.length > 0) && mdaos.map((item, index) => (
                                            <Col xs={12} md={4} xl={3} key={index} className="my-3 ">
                                                <div
                                                    className={`text-center py-3 ecosystem ${(mselected && mselected.address === item.address) && 'selectedecosystem'}`}
                                                    style={{ outline: 'none' }}
                                                    onClick={() => mselectedDao(item)}

                                                >
                                                    <div className="text-center mb-3" style={{ maxWidth: '100%' }}>
                                                        <GetImage
                                                            className='img-fluid'
                                                    ...
                                                    >{truncateAddress(item.address)}</div>
...
const formSchema = yup.object().shape({
    senderAddress: yup.string().test('isAddres', 'Invalid address', function (value) {
        if (value) {
