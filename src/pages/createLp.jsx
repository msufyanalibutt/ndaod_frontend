...
import { ShopLp_contract_address } from '../utils'
...
const CreateLp = () => {
    ...
    const handleFormSubmit = async ({ title, description, lpName, lpSymbol }) => {
        if (!library) return;
        try {
            setLoading(true)
            const contract = await dao(address);
            let timestamp = dayjs().unix();
            let iface = createForIface(lpName, lpSymbol);
    ...
    const instanceHandleSubmit = async () => {
        const { title, description, lpName, lpSymbol }= values;
        if (!library) return;
        try {
            setiLoading(true)
            const contract = await dao(address);
    ...
    const {
        ...
        initialValues: { title: "", description: "", lpName: "", lpSymbol: "" },
        validationSchema: formSchema
    });
    const createForIface = (_lpName, _lpSymbol) => {
        let ABI = ["function createLp(string memory _lpName,string memory _lpSymbol)"];
        let iface = new ethers.utils.Interface(ABI);
        iface = iface.encodeFunctionData("createLp", [_lpName, _lpSymbol]);
        return iface
    }
                        ...
                        <Col xs={12} md={8} className="mx-auto text-white">
                            ...
                            <Form onSubmit={handleSubmit}>
                                ...
                                <FormGroup className='mb-3' >
                                    <FormLabel htmlFor="lpName">LP Name</FormLabel>
                                    <FormControl
                                        id="lpName"
                                        type="text"
                                        name="lpName"
                                        onChange={(e) => {
                                            handleChange(e);
                                            setFieldValue('lpSymbol', e.target.value.toUpperCase());
                                            if (e.target.value.length > 0)
                                                setFieldValue('title', `Create LP Token ${e.target.value} (${e.target.value.toUpperCase()})`)
                                        }}
                                        onBlur={handleBlur}
                                        value={values.lpName || ""}
                                    />
                                    <small className="text-danger">{touched.lpName && errors.lpName}</small>
                                </FormGroup>
                                <FormGroup className='mb-3' >
                                    <FormLabel htmlFor="lpSymbol">LP Symbol</FormLabel>
                                    <FormControl
                                        id="lpSymbol"
                                        type="text"
                                        name="lpSymbol"
                                        ...
                                        onBlur={handleBlur}
                                        value={values.lpSymbol || ""}
                                    />
                                    <small className="text-danger">{touched.lpSymbol && errors.lpSymbol}</small>
                                </FormGroup>
                                <FormGroup className='mb-3' >
...
const formSchema = yup.object().shape({
    lpName: yup.string().min(3, 'Too Short!').required("Lp Name is required"),
    lpSymbol: yup.string().min(3, 'Too Short!').required("Lp Symbol is required"),
