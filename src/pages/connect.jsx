...
const Connect = () => {
    ...
    const { daoList, getDaoViewerContract } = WALLETCONTEXT();
    ...
    useEffect(() => {
        if (connector && active && account) {
            ...
            if (connector._connected) {
                getDaoInfo(connector._accounts[0])
            } else {
                getDaosList();
            }
        }
    ...
    const getDaoInfo = async (address) => {
        try {
            setLoading(true)
            const contract = await getDaoViewerContract();
            const result = await contract.getDao(address);
            setDao({
                dao: result.dao,
                daoName: result.daoName,
                daoSymbol: result.daoSymbol
            })
            setLoading(false)
        } catch (error) {
        ...
        if (connector) {
            ...
            connector.on("call_request", async (error, payload) => {
                ...
                if (payload.params.length > 0) {
                    ...
                    // navigate(`/customTransaction/${payload.params[1]}?targetAddress=${payload.params[1]}&data=${pload}&value=${'0x00'}&desc=${connector.peerMeta.description}`)
                    navigate(`/customTransaction/${pload.from}?targetAddress=${pload.to}&data=${pload.data ? pload.data : ''}&value=${pload.value ? pload.value : '0x00'}&desc=${connector.peerMeta.description}`)
                }
            });
        }
                ...
                <Container className='py-3'>
                    <Row>
                        <Col xs={12} xl={8} className="mx-auto text-white">
                            <h1>Connect</h1>
                            <p>Connect your DAO to any DeFi project. Possibilities are limitless.</p>
                                ...
                                <Row xs={1} sm={2} md={3}>
                                    {
                                        daos.map((dao, index) => (
                                            ...
                                            <Col key={index} className="mb-3">
                                                <div className={`d-flex align-items-center py-2 px-3 ecosystem ${selectedDao === dao.dao && 'selectedecosystem'}`} onClick={() => HandleSelectedDao(dao.dao)}>
                                                    <div className='profile me-3' style={{ width: '40px', height: '40px', background: randomColor() }}>
                                                    </div>
                                                    <div>
                                                        <p className='p-0 m-0'>
                    ...
                    <ModalBody>
                        ...
                        <Row>
                            ...
                            <Col className="mb-3" xs={12} sm={7}>
                                <p className='text-center p-0 m-0'> Come back, paste the URI, and connect your DAO</p>
                            </Col>
                        </Row>
                    </ModalBody>
                </Modal>
