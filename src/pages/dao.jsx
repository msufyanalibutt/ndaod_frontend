...
import { covalent, index_contract_address } from '../utils';
...
const DAO = () => {
    ...
    const { dao, getDaoViewerContract, getLpContract, getShopTaContract } = WALLETCONTEXT();
    ...
    const getDao = async (address) => {
        try {
            ...
            const daoconf = await contract.getDaoConfiguration(index_contract_address[chainId], address);
            setDaoConfig({
                ...
                lpAddress: constants.AddressZero === daoconf.lpAddress ? null : daoconf.lpAddress
            })
            getLpMembers(constants.AddressZero === daoconf.lpAddress ? null : daoconf.lpAddress);
        } catch (error) {
        }
    }
    ...
    const getLpMembers = async (lpMembers) => {
        const url = `https://api.covalenthq.com/v1/${chainId}/tokens/${lpMembers}/token_holders/?key=${covalent}`;
        try {
            const result = await axios.get(url);
            let items = result.data.data.items;
            setLpMembers(items);
        ...
        <>
            {
                            ...
                            <Col xs={12} lg={9} className="mx-auto">
                                <Row>
                                    <Col className='d-flex align-items-center  text-white'>
                                        ...
                                        <div>
                                            <h4>{name}</h4>
                                            <h4>{truncateAddress(address)} <ClipBoard address={address} /></h4>
                                        </div>
                                    </Col>
                                    <Col >
                                        <div
                                        ...
                                        >
                                            <button className='dao-primary-btn py-2 px-3 w-100 mb-2'>Change Avatar</button>
                                ...
                                <div className="text-white p-3 mb-3 tabborder">
                                    <Row>
                                        ...
                                        <Col className='text-center' >
                                            <h6>AUM</h6>
                                            <h2>
                                                ${new Intl.NumberFormat('en-US', { maximumFractionDigits: 2, notation: "compact", compactDisplay: "short" }).format(daoBalance)}
                                            </h2>
                                        </Col>
                                    </Row>
                                </div>
                        ...
                        <Row>
                            <Col xs={12} lg={9} className="mx-auto">
                                <Tabs
                                    ...
                                    className="mb-3"
                                >
                                    <Tab eventKey="balance" title={<><FaWallet className='icon' /> <span>Balance</span></>}>
                                        <div className='myassets text-white tabborder mb-3 p-3'>
                                            {
                                                account && <MyAssets owner={owner} daoBalance={daoBalance} assets={assets} chainId={chainId} address={address} account={address} covalent={covalent} />
                                            }
                                        </div>
                                        <div className='text-muted p-3 mb-3 tabborder'>
                                            {
                                                account && <Transactions chainId={chainId} address={address} account={address} covalent={covalent} />
                                            }
                                        </div>
                                    </Tab>
                                    <Tab eventKey="lp" title={<><IoIosPeople className='icon' /> <span>LP Token</span></>}>
                                        <Row xs={1} sm={1} md={1}>
                                            <Col className="mb-5">
                                                {
                                                    daoConfig.lpAddress &&
                                                    <Row>
                                                        <Col className='mb-3'>
                                                            <span className='text-white'>LP Token: {truncateAddress(daoConfig.lpAddress)} <ClipBoard address={daoConfig.lpAddress} /></span>
                                                        </Col>
                                                    ...
                                                    <h5>Offers</h5>
                                                    {
                                                        (!daoConfig.lpAddress && owner) && <>
                                                            <div className="text-center py-5">
                                                                <span>Create Liquidity Provider token to deposit funds in this DAO</span>
                                                            </div>
                                                            <Row>
                                                                <Col>
                                                                ...
                                                                <Col className='text-right'>
                                                                    <NavLink to={`/createLp/${address}`} className="dao-btn">Create LP</NavLink>
                                                                </Col>
                                                            </Row>
                                                        </>
                                                    }
                                                    {
                                                        daoConfig.lpAddress && <LPToken {...daoConfig} name={name} address={address} assets={assets} />
                                                    }
                                                </div>
                                            </Col>
                                        </Row>
                                    </Tab>
                                    <Tab eventKey="TA" title={<><IoIosPeople className='icon' /> <span>Trading Account</span></>}>
                                        <Row xs={1} sm={1} md={1}>
                                            <Col className="mb-5">
                                                <div className='text-white p-3 mb-3 tabborder h-100 '>
                                                    <Row>
                                                        ...
                                                        </Col>
                                                        {
                                                            owner && <Col className='text-right'>
                                                                <NavLink to={`/createTa/${address}`} className="dao-btn d-inline-block">Create Account</NavLink>
                                                            </Col>
                                                        }
                                                    </Row>
                                                    {
                                    ...
                                    <Tab eventKey="voting" title={<><HiSpeakerphone className='icon' /><span>VOTING</span></>}>
                                        <div className='text-muted p-3 mb-3 tabborder'>
                                            <Row className='mb-2'>
                                                <Col>
                                                ...
                                                {
                                                    owner && <Col className='text-right pt-2 mb-3'>
                                                        <NavLink to={`/votingList/${address}`} className={"dao-btn"} >Create Voting</NavLink>
                                                    </Col>
                                                }
                                            </Row>
                                            {
                                    ...
                                    <Tab eventKey="tokenholders" title={<span>TOKEN HOLDERS</span>}>
                                        <div className='text-white p-3 py-4 mb-3 tabborder'>
                                            <Row>
                                                ...
                                                <Col className='text-right'>
                                                    {
                                                        owner && <NavLink to={`/mintGt/${address}`} className='dao-btn'>
                                                            <BsFillPersonPlusFill className='icon' />Add Member
                                                        </NavLink>
                                                    }
                                                </Col>
                                            </Row>
                                                ...
                                                <Table >
                                                    <thead>
                                                        <tr className='border-0'>
                                                            <th className='text-white bg-transparent'>address</th>
                                                            <th className='text-white bg-transparent text-right'>GT Balance</th>
                                                        </tr>
                                                    </thead>
                                                    <tbody>
                                                        {
                                                            (members && members.length > 0) && <>
                                                                {
                                                                    members.map((member, index) => (
                                                                        <tr className='border-0' key={index}>
                                                                            <Members member={member} address={address} owner={owner} />
                                                                        </tr>
                                                                    ))
                                                                }
                                                            </>
                                        ...
                                        {
                                            daoConfig.lpAddress && <div className='text-white p-3 py-4 mb-3 tabborder'>
                                                <Row>
                                                    <Col>
                                                ...
                                                <Container className='py-3'>
                                                    <Table >
                                                        <thead>
                                                            <tr className='border-0'>
                                                                <th className='text-white bg-transparent'>address</th>
                                                                <th className='text-white bg-transparent text-right'>LP Balance</th>
                                                            </tr>
                                                        </thead>
                                                        <tbody>
                                                            {
                                                                (lpMembers && lpMembers.length > 0) && <>
                                                                    {
                                                                        lpMembers.map((member, index) => (
                                                                            <tr className='border-0' key={index}>
                                                                                <LpMember member={member} address={member.address} owner={false} />
                                                                            </tr>
                                                                        ))
                                                                    }
                                                                </>
                                    ...
                                    <Tab eventKey="features" title={<span>Features</span>}>
                                        {
                                            active && <DaoConfiguration {...daoConfig} name={name} daoAddress={address} />
                                        }
                                    </Tab>
                ...
                </div> : <div className='text-center my-5 mx-auto' style={{ maxWidth: '300px' }}>
