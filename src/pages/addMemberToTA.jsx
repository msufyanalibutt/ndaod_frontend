...
import { Container, Row, Col, FormControl, Form, FormGroup, FormLabel, InputGroup, Spinner, Modal, ToastContainer } from 'react-bootstrap';
...
const AddMemberToTA = () => {
    ...
    const handleFormSubmit = async ({ title, recipientAddress, tokenAddress, description }) => {
        if (!library) return;
        try {
            setLoading(true);
            const contract = await dao(address);
            let timestamp = dayjs().unix();
            let iface = createForIface(recipientAddress)
    ...
    const createForIface = (to) => {
        let ABI = ["function mint(address _to)"];
        let iface = new ethers.utils.Interface(ABI);
        iface = iface.encodeFunctionData("mint", [to]);
        return iface
    }
                            ...
                            <Form onSubmit={handleSubmit}>
                                ...
                                <FormGroup className='mb-3'>
                                    ...
                                    <button type="button" className='dao-btn w-100' onClick={handleShow}>
                                        {
                                            values.tokenAddress === '' ? 'Choose Token' : <Row>
                                ...
                                <FormGroup className='mb-3'>
                                    <FormLabel htmlFor="recipientAddress" >Trader Account Address</FormLabel>
                                    <FormControl
                                        type="text"
                                        id="recipientAddress"
                            ...
                            </Row>
                            {
                                assets.map((item, index) => (
                                    <div
                                        ...
                                        key={index} onClick={() => handleManualInputAddress(item)}>
                                        <Row>
