...
const SendToken = () => {
  ...
  const handleFormSubmit = async ({
    ...
    try {
      ...
      iface = createForTAIface(daoToken, iface, 0);
  ...
  const createForTAIface = (target, data, value) => {
    let ABI = [
      "function widthDraw(address _target,bytes calldata _data,uint256 _value)",
    ];
    let iface = new ethers.utils.Interface(ABI);
  ...
  const getDaoAssets = async () => {
    ...
    try {
      ...
      items = items.filter(
        (item) => item.token_address !== exlcude_Address[chainId]
      );
      setAssets(items);
    } catch (error) {}
  };
  const maxTokenTransfer = () => {
    ...
    if (tokenBalance > 0 && values.recipientAddress) {
      setFieldValue(
        "title",
        `Send ${tokenBalance} ${symbol} to ${values.recipientAddress}`
      );
    }
  };
  const formSchema = yup.object().shape({
    ...
    amount: yup
      ...
      .test(
        "maxLength",
        `You must have enough ${symbol} in addition to the amount to pay for gas`,
        function (value) {
      ...
      .test(
        "maxLength",
        `Low balance. Current balance is ${tokenBalance} ${symbol}`,
        function (value) {
            ...
            <Col xs={12} md={8} className="mx-auto text-white">
              ...
              <Form onSubmit={handleSubmit}>
                <FormGroup className="mt-3">
                  <p>Transfer Any Token from Trading Account balance to Dao</p>
                  <hr />
                </FormGroup>
                <FormGroup className="mb-3">
                  ...
                  <button
                    ...
                    onClick={handleShow}
                  >
                    {values.tokenAddressValue === -1 ? (
                      "Choose Token"
                    ) : values.tokenAddressValue === 0 ? (
                      ...
                      <Row>
                        <Col className="d-flex align-items-center">
                          <div className="me-3">
                            <img
                              src={
                                assets[values.tokenAddressValue - 1].logo
                              }
                              alt={
                                assets[values.tokenAddressValue - 1]
                                  .name
                              }
                        ...
                        <Col className="text-right">
                          <p className="m-0 p-0">
                            {Number(
                              assets[values.tokenAddressValue - 1].balance /
                                Math.pow(
                                  10,
                                  assets[values.tokenAddressValue - 1]
                                    .decimals
                ...
                {values.tokenAddressType !== "hidden" && (
                  <FormGroup className="mb-3">
                  ...
                  <FormControl
                    ...
                    onChange={(e) => {
                      ...
                      if (values.amount > 0 && e.target.value) {
                        setFieldValue(
                          "title",
                          `Send ${values.amount} ${symbol} to ${e.target.value}`
                        );
                      }
                    }}
                    onBlur={handleBlur}
                    ...
                    <FormControl
                      ...
                      onChange={(e) => {
                        ...
                        if (e.target.value > 0 && values.recipientAddress) {
                          setFieldValue(
                            "title",
                            `Send ${e.target.value} ${symbol} to ${values.recipientAddress}`
                          );
                        }
                      }}
                      onBlur={handleBlur}
            ...
            <Modal.Body className="py-3">
              ...
              {assets.map((item, index) => (
                <div
                  ...
                  className="py-2 px-3 pointer permitted-remove"
                  key={index}
                  onClick={() => handleManualInputAddress(item, index + 1)}
                >
                  <Row>
                    <Col className="d-flex align-items-center">
                      <div className="me-3">
