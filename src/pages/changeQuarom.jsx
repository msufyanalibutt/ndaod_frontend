...
const ChangeQuorom = () => {
    ...
    const [oldQuorum, setOldQuorum] = useState(0);
    ...
    const getDaoInfo = async () => {
        try {
            ...
            let result = await contract.quorum();
            setOldQuorum(result);
        } catch (error) {
        }
    }
    const handleFormSubmit = async ({ title, quorum, description }) => {
        if (!library) return;
        try {
            setLoading(true)
            const contract = await dao(address);
            let timestamp = dayjs().unix();
            let iface = createForIface(quorum)
    ...
    const instanceHandleSubmit = async () => {
        const { title, quorum, description } = values;
        if (!library) return;
        try {
            setiLoading(true)
            const contract = await dao(address);
    ...
    const {
        ...
        initialValues: { title: "", description: "", quarom: 0 },
        validationSchema: formSchema
    });
    const createForIface = (_quorom) => {
        let ABI = ["function changeQuorum(uint8 amount)"];
        let iface = new ethers.utils.Interface(ABI);
        iface = iface.encodeFunctionData("changeQuorum", [_quorom]);
        return iface
    }
                        ...
                        <Row>
                            <Col xs={12} md={8} className="mx-auto text-white">
                                ...
                                <Form onSubmit={handleSubmit}>
                                    ...
                                    <FormGroup className='mb-3'>
                                        <FormLabel htmlFor='quorum'>New Quorom</FormLabel>
                                        <FormControl
                                            id="quorum"
                                            type="number"
                                            name="quorum"
                                            ...
                                            onChange={(e) => {
                                                ...
                                                if (e.target.value && e.target.value > 0) {
                                                    setFieldValue('title', `Change Quorum from ${oldQuorum}% to ${e.target.value}%`)
                                                }
                                            }}
                                            onBlur={handleBlur}
                                            value={values.quorum || ''}
                                        />
                                        <small className="text-danger">{touched.quorum && errors.quorum}</small>
                                    </FormGroup>
                                    <FormGroup className='mb-3' >
...
const formSchema = yup.object().shape({
    ...
    quorum: yup.number().moreThan(0, 'Must be more then 0').test('maxLength', 'Must be range from 1 to 100', function (value) {
        if (value <= 100) {
            return true;
        } else {
            return false
