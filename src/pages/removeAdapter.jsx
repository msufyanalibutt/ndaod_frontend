...
const RemoveAdapter = () => {
    ...
    useEffect(() => {
        ...
        if (target) {
            ...
            setFieldValue('title', `Remove Adapter: ${target}`);
        }
    }, [])
    useEffect(() => {
    ...
    const createForIface = (a) => {
        let ABI = ["function removeAdapter(address a)"];
        let iface = new ethers.utils.Interface(ABI);
        iface = iface.encodeFunctionData("removeAdapter", [a]);
        return iface
    }
    ...
    const getAdapterList = async () => {
        try {
            const contract = await dao(address);
            const adapterList = await contract.getAdapters();
            setAdapterList(adapterList)
        } catch (error) {
        }
    }
    const handleAdapter = (item) => {
        setFieldValue('targetAddress', item);
        setFieldValue('title', `Remove Adapter: ${item}`);
        handleClose();
    }
    return (
        <>
                ...
                active ? <>  <Container className='my-3'>
                    <Row>
                        <Col xs={12} lg={8} className="mx-auto text-white">
                            <h2>Remove Adapter</h2>
                            <p>Remove Adapter from your DAO.</p>
                            ...
                            </Row>
                            {
                                adapterList.map((item, index) => (
                                    <div
                                        ...
                                        key={index} onClick={() => handleAdapter(item)}>
                                        {item}
                                    </div>
                                ))
                            }
